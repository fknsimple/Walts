...
- (id)init
{
    ...
    if (self){
        ...
        self.frameImageView.clipsToBounds = YES;
    }
    return self;
}
...
- (void)viewDidLoad
{
    ...
    if(_imageView == nil){
        ...
        [_scrollView addSubview:self.frameImageView];
        [self refreshImageView];
    }
}
...
//        _imageView.contentMode = UIViewContentModeScaleToFill;
//        _imageView.frame = CGRectMake(0, _scrollView.frame.size.height/2, _scrollView.frame.size.width, 753);
//        _imageView.center = _scrollView.center;

        self.frameImageView.frame = _imageView.frame;
        self.frameImageView.image = [UIImage imageWithData:[[NSUserDefaults standardUserDefaults] valueForKey:@"FRAMEIMAGE"]];
        self.frameImageView.center = _imageView.center;
    }
}
